...
export default function AccessoriesInventoryPage() {
    ...
    // Handlers to open the modal correctly
    const openAddModal = () => {
        setEditingAccessory(null); // Clear the editing state to trigger an Add
        setIsModalOpen(true);
    };

    const openEditModal = (item: any) => {
        setEditingAccessory(item); // Pass the item down to trigger an Edit
        setIsModalOpen(true);
    };
    ...
    return (
        <div className="space-y-6 pb-12 h-full flex flex-col">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {accessories.map((item) => (
                        <div key={item.id} className="bg-zinc-950 border border-zinc-800/60 rounded-xl flex flex-col overflow-hidden hover:border-zinc-700 transition-colors shadow-sm group">
                            ...
                            {/* IMAGE AREA */}
                            <div className="relative h-48 bg-zinc-900 flex items-center justify-center border-b border-zinc-800/60 overflow-hidden">
                                {item.image_url ? (
                                    <img src={`http://127.0.0.1:8000${item.image_url}`} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                ) : (
            ...
            {/* Our external, perfectly reusable Modal Component! */}
            <AccessoryModal 
                isOpen={isModalOpen} 
